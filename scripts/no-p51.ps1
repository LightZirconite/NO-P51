...
function Get-Nop51GitStatusItems {
  param(
    [string[]]$Lines
  )
  ...
  foreach ($entry in $Lines) {
    ...
    $status = $normalized.Substring(0, 2)
    $pathSegment = if ($normalized.Length -gt 3) { $normalized.Substring(3) } else { "" }
    if ([string]::IsNullOrWhiteSpace($pathSegment)) {
      continue
    }

    $pathValue = $pathSegment.Trim()
    if ($pathValue -match "\s->\s") {
      $split = $pathValue -split "\s->\s"
      if ($split.Count -gt 0) {
        $pathValue = $split[-1].Trim()
      }
    }
...
function Test-Nop51AllowedGitPath {
  param(
    [string]$Path
  )
  ...
  $normalized = $Path.Trim().ToLowerInvariant().Replace("\\", "/")
  $allowed = @("config.json", "config.local.json")
